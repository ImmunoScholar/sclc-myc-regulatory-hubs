# M7 — SCLC-selective CRISPR dependency

Generated: 2026-07-27 18:42:35 UTC
DepMap Public 26Q1: 25 SCLC lines vs 1183 other lineages.

Dependency is **not expression**, so unlike the transcriptional and
tumour-network domains it cannot inherit the neuroendocrine lineage
confound that removed them (D-033).

Genes essential in SCLC (effect <= -0.5): **1,701**; selectively so: **84**.

| paralog | regulon genes | selective | rate in | rate out | OR | p | programme |
|---|---|---|---|---|---|---|---|
| MYC | 373 | 4 | 1.07% | 0.44% | 2.45 | 0.0894 | NEUROGENESIS |
| MYCN | 413 | 4 | 0.97% | 0.44% | 2.21 | 0.118 | HALLMARK |
| MYCL1 | 375 | 5 | 1.33% | 0.44% | 3.09 | 0.0278 | NONE |
